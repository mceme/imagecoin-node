...
var AxedRPC = require('@mceme/imagecoin-rpc');
...
Axe.prototype.subscribe = function(name, emitter) {
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'ImageCoind/' + name, 'total:', this.subscriptions[name].length);
};
...
Axe.prototype.unsubscribe = function(name, emitter) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'ImageCoind/' + name, 'total:', this.subscriptions[name].length);
};
...
Axe.prototype.subscribeAddress = function(emitter, addresses) {
  ...
  log.info(emitter.remoteAddress, 'subscribe:', 'ImageCoind/addresstxid', 'total:', _.size(this.subscriptions.address));
};
...
Axe.prototype.unsubscribeAddress = function(emitter, addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'ImageCoind/addresstxid', 'total:', _.size(this.subscriptions.address));
};
...
Axe.prototype._loadSpawnConfiguration = function(node) {
  ...
  $.checkArgument(this.options.spawn, 'Please specify "spawn" in ImageCoind config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in ImageCoind config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in ImageCoind config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './imagecoin.conf');

  log.info('Using ImageCoin config file:', configPath);
...
Axe.prototype._zmqBlockHandler = function(node, message) {
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('ImageCoind/hashblock', message.toString('hex'));
    }
  }
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('ImageCoind Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('ImageCoind/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Axe.prototype._zmqTransactionHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('ImageCoind/rawtransaction', message.toString('hex'));
    }
...
Axe.prototype._zmqTransactionLockHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactionLocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.transactionlock.length; i++) {
      this.subscriptions.transactionlock[i].emit('ImageCoind/transactionlock', message.toString('hex'));
    }
...
Axe.prototype._stopSpawnedAxe = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/ImageCoind.pid';
...
Axe.prototype._spawnChildProcess = function(callback) {
  ...
  self._stopSpawnedAxe(function(err) {
    ...
    log.info('Starting ImageCoin process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('ImageCoin process unexpectedly exited with code:', code);
        log.warn('Restarting ImageCoin child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('ImageCoin process restarted');
          });
...
Axe.prototype._connectProcess = function(config, callback) {
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to ImageCoind.'));
    }
  ...
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('ImageCoind spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('ImageCoind process did not exit'));
      }
