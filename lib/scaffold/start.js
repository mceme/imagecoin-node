...
var imagecoinNode = require('../node');
var index = require('../');
        ...
        servicesConfig: {
          Imagecoind: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/Imagecoind')
            }
          }
        }
      };
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './imagecoin-node.json');
  ...
  var node = new imagecoinNode(fullConfig);
...
function loadModule(req, service) {
  try {
    ...
    if (servicePackage.imagecoinNode) {
      serviceModule = service.name + '/' + servicePackage.imagecoinNode;
    }
